...
class LanguageTokensTest {
    ...
    @Test fun `test keywords`() {
        ...
        assertEquals(firstMatch(tokens, "zewnętrzna"), TokenType.FOREIGN1)
        assertEquals(firstMatch(tokens, "zewnętrzny"), TokenType.FOREIGN2)
        assertEquals(firstMatch(tokens, "jako"), TokenType.AS)
        assertEquals(firstMatch(tokens, "przekaźnik"), TokenType.GENERATOR)
        assertEquals(firstMatch(tokens, "przekaż"), TokenType.YIELD)
        assertEquals(firstMatch(tokens, "otrzymując"), TokenType.FOR_EACH)
        assertEquals(firstMatch(tokens, "od"), TokenType.FROM)
